  ...
  <div class="table-page">
    ...
    >
      ...
      <template #toolbar-right>
        ...
        <!-- 自定义已读和删除 -->
        <a-button type="primary" :disabled="!selectedKeys.length" @click="onReadOwer">已读</a-button>

        <a-button  type="primary" status="danger" :disabled="!selectedKeys.length" @click="onDeleteOwer">
          <template #icon><icon-delete /></template>
          <span>删除</span>
        </a-button>

        <a-button type="primary"  :disabled="selectedKeys.length" @click="onReadAll">全部已读</a-button>
        <a-button v-permission="['setting:message:delete']" type="primary" status="danger" :disabled="!selectedKeys.length" @click="onDelete">
          <template #icon><icon-delete /></template>
          <span>全体删除</span>
        </a-button>
      </template>
...
import { type MessageQuery, deleteMessage, listMessage,readMessage, readMessageOwer,deleteMessageOwer } from '@/apis'
...
const onDeleteOwer = () => {
  ...
  return handleDelete(() => deleteMessageOwer(selectedKeys.value), { showModal: false })
}
...
// 标记当前用户为已读
const onReadOwer = async () => {
  if (!selectedKeys.value.length) {
    return Message.warning('请选择数据');
  }
  try {
    const response = await readMessageOwer(selectedKeys.value);
